...
export const REGIONAL_RSS_SOURCES = [
  ...
  { name: 'Zonet Cable TV', state: 'Mizoram', url: 'https://zonet.in/feed', sourceUrl: 'https://zonet.in' },
  { name: 'Hornbill TV', state: 'Nagaland', url: 'https://hornbilltv.com/feed', sourceUrl: 'https://hornbilltv.com' },
  ...
  async run(): Promise<number> {
    ...
    for (const regSource of REGIONAL_RSS_SOURCES) {
      ...
      if (regArticles.length === 0) {
        console.log(`[Ingestion] Feed for ${regSource.name} (${regSource.state}) empty/failed. Skipping mock generation.`);
      }
