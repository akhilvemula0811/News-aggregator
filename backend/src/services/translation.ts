...
${JSON.stringify(chunk.map(c => c.text), null, 2)}`;
  ...
  if (remainingToTranslate.length > 0) {
    console.log(`[Translation] Using Batch Google Translate API (Mobile App Endpoint) for ${remainingToTranslate.length} remaining items`);
    const chunkSize = 20;
    for (let i = 0; i < remainingToTranslate.length; i += chunkSize) {
      ...
      try {
        ...
        const url = `https://translate.google.com/translate_a/single?client=at&sl=auto&tl=${apiLangCode}&dt=t&q=${encodeURIComponent(combinedText)}`;
        ...
        for (const item of chunk) {
          try {
            const apiLangCode = langCode === 'mni' ? 'mni-Mtei' : langCode;
            const url = `https://translate.google.com/translate_a/single?client=at&sl=auto&tl=${apiLangCode}&dt=t&q=${encodeURIComponent(item.text)}`;
            const response = await axios.get(url, {
              headers: {
                'User-Agent': 'AndroidTranslate/5.3.0.RC02.130751341 (Linux; U; Android 6.0; redmi 3s)'
              },
